...
// connect
mongoose.connect('mongodb://localhost/vnwa', err => {
    if (err) { 
        throw err; 
    }
});
...
// create user
exports.createUser = (userName, password, isAdmin) => {
...
// get specific user
exports.getUser = (name, callback) => {
    userModel.find({username: name}, (err, user) => {
        if (err) {
            throw err;
        }
...
exports.changePassword = (userName, newPassword) => {
    ...
    userModel.update({username : userName}, {password : newPassword}, {multi : true}, err => {
        if (err) {
            throw err; 
        }
...
// create message
exports.createMessage = (fromUser, toUser, contentMessage) => {
...
exports.getMessages = (toUser, callback) => {
    console.log(toUser);
    messageModel.find({to: toUser}, (err, messages) => {
        if (err) {
            throw err;
        }
